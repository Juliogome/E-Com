...
if page == pages[1] :
  ...
  st.image("events.png", width=None)
  st.write("**Category Dataframe:**")
  st.image("category_tree.png", width=None)
  st.write("**Item Property Part 1:**")
  st.image("item_properties_part1.png", width=None)
  st.write("**Item Property Part 2:**")
  st.image("item_properties_part2.png", width=None)
...
if page == pages[2] : 
  ...
  st.write("The pie chart shows a clear imbalance between the three events. This was expected as it follows the natural progress of a customer journey on an e-commerce site. Though, further analysis may be needed to varify that there are enough add2cart events to accurately predict purchase behaviour. ")

  st.subheader("Event Distribution by Hour")

  st.write("Events are analyzed based on the hour of the day and the day of the week they occur, with separate visualizations for events categorized as 'view', 'add_to_cart', and 'transaction'. These analyses reveal peak activity hours and weekly user engagement patterns, providing valuable insights for scheduling marketing campaigns and platform maintenance. ")

  st.image("events by hour.png", width=None)

  st.write("The graph above shows that visitors and customers are mainly active between 4 pm and 5 am. There is no information in which time zone or time zones the data was collected.")

  st.subheader("Event Distribution by Weekday")

  st.write("The graph below shows that visitors and customers are more active during the week from Monday to Friday than on the weekend.")

  st.image("events by weekday.png", width=None)

  st.write("For a better visual comparison, the graph is further divided into event “view” and events “addtocart” and “transaction”.")

  st.image("events by hour - views.png", width=None)
  st.image("events by hour - add2cart_transaction.png", width=None)
  st.image("events by weekday - views.png", width=None)
  st.image("events by weekday - add2cart_transaction.png", width=None)

  st.write("It can be observed that the higher the number of views per hour or per day, the higher the number of 'addtocart' events, and therefore the higher the number of 'transaction' events. A correlation between these variables can be assumed.")

  st.subheader("Top 5 users")

  st.write("This visualization provides insights into the top 5 users based on their interactions with the e-commerce platform. The analysis begins by aggregating the data to count the number of events per user for each event type. Subsequently, the top 5 user IDs for each event type are identified based on the number of events they have generated. The bar plot illustrates the top 5 user IDs per event type, with each event type represented by a distinct color. Users are ranked based on the number of events they have performed, offering visibility into the most active users across different types of interactions. Additionally, individual bar plots are generated for each event type ('view', 'addtocart', 'transaction'), showcasing the top 5 user IDs specific to each event type.")
  
  st.image("top 5 users by event type.png", width=None)

  st.image("top 5 users - view.png", width=None)

  st.image("top 5 users - add2cart.png", width=None)

  st.image("top 5 users - transactions.png", width=None)
...
if page == pages[3]:
    ...
    st.image("model-overview.png", width=None)
...
if page == pages[5] : 
  ...
  st.image("conclusion.jpg", width=None)
  st.write("""Limitation
